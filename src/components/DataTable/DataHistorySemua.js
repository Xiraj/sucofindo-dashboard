...
export default function DataHistorySemua() {
  ...
  const getData = async () => {
    try {
      const response = await axios
        .get("https://sima-rest-api.vercel.app/api/v1/data/history", {
          headers: {
  ...
  const Filter = (event) => {
    ...
    const filtered = data.filter((item) => {
      const jenisPengembalian = item.id_pengembalian?.jenis || "";
      const jenisPeminjaman = item.id_peminjaman?.jenis || "";
      const namaAsetPengembalian =
        item.id_pengembalian?.id_aset?.nama_alat || "";
      const namaAsetPeminjaman = item.id_peminjaman?.id_aset?.nama_alat || "";
      const nama = item.id_user?.username || "";
      return (
        jenisPengembalian.toLowerCase().includes(searchTerm) ||
        jenisPeminjaman.toLowerCase().includes(searchTerm) ||
        namaAsetPengembalian.toLowerCase().includes(searchTerm) ||
        namaAsetPeminjaman.toLowerCase().includes(searchTerm) ||
  ...
  const convertToXLS = () => {
    try {
      ...
      const dataToExport = data.map((item) => ({
        ...
        "Tag Number":
          item.id_pengembalian?.id_aset?.tag_number ||
          item.id_peminjaman?.id_aset?.tag_number,
        Merek:
          item.id_pengembalian?.id_aset?.merek ||
          item.id_peminjaman?.id_aset?.merek,
        Tipe:
          item.id_pengembalian?.id_aset?.tipe ||
          item.id_peminjaman?.id_aset?.tipe,
        "Nomor Seri":
          item.id_pengembalian?.id_aset?.nomor_seri ||
          item.id_peminjaman?.id_aset?.nomor_seri,
        "Penanggung Jawab Aset":
          item.id_pengembalian?.id_aset?.penanggung_jawab ||
          item.id_peminjaman?.id_aset?.penanggung_jawab,
        "Lokasi Aset":
          item.id_pengembalian?.lokasi || item.id_peminjaman?.lokasi,
        "Kondisi Aset":
          item.id_pengembalian?.kondisi_aset ||
          item.id_peminjaman?.kondisi_aset,
        Tanggal:
          item.id_pengembalian?.tanggal_pengembalian ||
          item.id_peminjaman?.tanggal_peminjaman,
        "Diajukan Oleh": item.id_user?.username,
        Admin: item.id_admin?.username,
        Pengajuan: item.id_pengembalian?.jenis || item.id_peminjaman?.jenis,
        Status: item.id_pengembalian?.status || item.id_peminjaman?.status,
      }));
        ...
        <div className="flex justify-center">
          ...
          >
            <div className="flex justify-between">
              ...
              <PiExportBold
                className="w-[2rem] h-[2rem] text-white pr-3"
                size={24}
              />
            </div>
          </button>
            ...
            <table>
              <thead className="font-bold w-[66.5rem] h-[3.5rem] bg-[#F3F3F3]">
                <tr>
                  ...
                  <td className="w-[78.625rem] pl-[1rem] py-2  border-y-2 border-[#e8e8e8]">
                    Pengajuan
                  </td>
              ...
              <tbody>
                {records.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="w-[68.625rem] pl-[1rem] h-[3.5rem] border-y-2 border-[#e8e8e8]">
                      {item.id_pengembalian?.jenis || item.id_peminjaman?.jenis}
                    </td>
                    <td
        ...
        <ul className="grid justify-items-stretch pagination">
          <div className="justify-self-start">
            <label className="flex relative top-6">
              ...
              <p className="ml-4">Menampilkan Riwayat Barang Masuk</p>
            </label>
          </div>
          <div className="flex justify-center ">
