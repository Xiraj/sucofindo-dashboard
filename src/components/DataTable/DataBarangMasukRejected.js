...
export default function DataTableMasukRejected() {
    ...
    useEffect(() => {
        getData('Rejected'); // Fetch and display data with 'Decline' status
    }, []);
                ...
                <tbody>
                    {records.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td className='w-[98.625rem] h-[3.5rem] text-red-500 border-r-2 border-y-2 border-[#e8e8e8]'>{item.status}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
