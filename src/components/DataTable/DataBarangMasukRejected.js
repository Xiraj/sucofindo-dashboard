...
export default function DataTableMasukRejected() {
  ...
  const getData = async (status) => {
    try {
      const response = await axios.get(
        `https://sima-rest-api.vercel.app/api/v1/aset/listPengembali?status=${status}`,
        {
          headers: {
      ...
      const filteredData = response.data.pengembalian.filter(
            ...
            <table className="">
              <thead className="font-bold w-[66.5rem] h-[3.5rem] bg-[#F3F3F3]">
                <tr>
                  ...
                  <td className="w-[102.625rem] border-y-2 border-[#e8e8e8]">
                    Penanggung Jawab Aset
                  </td>
                  <td className="w-[98.625rem] border-y-2 border-[#e8e8e8]">
                    Lokasi Aset
                  </td>
                  <td className="w-[68.625rem] border-y-2 border-[#e8e8e8]">
                    Kondisi Aset
                  </td>
                  <td className="w-[79.625rem] border-y-2 border-[#e8e8e8]">
                    Tanggal Pengembalian
                  </td>
                  <td className="w-[78.625rem] border-y-2 border-[#e8e8e8]">
                    Diajukan Oleh
                  </td>
                  <td className="w-[48.625rem] border-y-2 border-[#e8e8e8]">
                    Status
                  </td>
                </tr>
              </thead>
              <tbody>
                {records.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="w-[148.625rem] h-[3.5rem] pl-[2rem] border-y-2 border-[#e8e8e8]">
                    ...
                    <td className="w-[50.625rem] h-[3.5rem] border-y-2 border-[#e8e8e8]">
                    ...
                    <td className="w-[98.625rem] h-[3.5rem] border-y-2 border-[#e8e8e8]">
                      {item.id_user.username}
                    </td>
                    <td className="w-[48.625rem] h-[3.5rem] text-red-500 border-y-2 border-[#e8e8e8]">
                      {item.status}
                    </td>
                  </tr>
                ))}
        ...
        <ul className="grid justify-items-stretch pagination">
          <div className="justify-self-start">
            <label className="flex relative top-6">
              ...
              <p className="ml-4">Menampilkan Data Barang</p>
            </label>
          </div>
