...
export default function DataTableKeluarApproved() {
  ...
  const getData = async (status) => {
    try {
      const response = await axios.get(
        `https://sima-rest-api.vercel.app/api/v1/aset/listPeminjam?status=${status}`,
        {
          headers: {
      ...
      const filteredData = response.data.peminjaman.filter(
  ...
  const Filter = (event) => {
    ...
    const filtered = data.filter((item) =>
      item.id_aset.nama_alat.toLowerCase().includes(searchTerm)
    );
            ...
            <table>
              <thead className="w-[86.5rem] h-[3.5rem] font-bold bg-[#F3F3F3]">
                <tr>
                  ...
                  <th className="w-[4.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                    Merek
                  </th>
                  ...
                  <th className="w-[80.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                    Penanggung Jawab Aset
                  </th>
                  <th className="w-[98.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                    Lokasi Aset
                  </th>
                  <th className="w-[48.625rem] border-y-2 border-[#e8e8e8] text-left">
                    Kondisi Aset
                  </th>
                  <th className="w-[58.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                    Tanggal Peminjaman
                  </th>
                  <th className="w-[48.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                    Tujuan Peminjaman
                  </th>
                  <td className="w-[48.625rem] border-r-2 border-y-2 border-[#e8e8e8]">
                    Status
                  </td>
                </tr>
              </thead>
                ...
                {records.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="w-[78.625rem] h-[3.5rem] border-y-2 border-[#e8e8e8]">
                      {item.id_aset.nama_alat}
                    </td>
                    <td className="w-[58.625rem] h-[3.5rem] border-y-2 border-[#e8e8e8]">
                      {item.id_aset.tag_number}
                    </td>
                    <td className="w-[54.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                      {item.id_aset.merek}
                    </td>
                    ...
                    <td className="w-[20.625rem] py-2 border-y-2 border-[#e8e8e8] text-left">
                      {item.id_aset.nomor_seri}
                    </td>
                    <td className="w-[80.625rem] h-[3.5rem] border-y-2 border-[#e8e8e8]">
                      {item.id_aset.penanggung_jawab}
                    </td>
                    <td className="w-[98.625rem] h-[3.5rem] pl-[1.25rem] border-y-2 border-[#e8e8e8]">
                      {item.id_aset.lokasi_aset}
                    </td>
                    ...
                    <td className="w-[48.625rem] h-[3.5rem] text-green-500 border-r-2 border-y-2 border-[#e8e8e8]">
                      {item.status}
                    </td>
                  </tr>
                ))}
