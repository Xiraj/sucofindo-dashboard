...
export default function Notification() {
  ...
  const getData = async () => {
    try {
      const peminjamanResponse = await axios.get(
        `https://sima-rest-api.vercel.app/api/v1/aset/listPeminjam`,
        {
          headers: {
      ...
      const pengembalianResponse = await axios.get(
        `https://sima-rest-api.vercel.app/api/v1/aset/listPengembali`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
  ...
  return (
    <div className="relative flex justify-center items-center cursor-pointer">
      <div
        className="w-9 h-14 pt-[0.8rem] pr-[0.2rem]"
      ...
      {notificationCount > 0 && (
        <div
          className={`absolute text-[12px] w-5 h-5 top-0 right-0 text-center justify-center items-center flex bg-red-500 text-white rounded-full px-2 ${
            showPopup ? "visible opacity-100" : "visible opacity-100"
          }`}
        >
          ...
          {showPopup && (
            <div className="absolute w-[270px] h-[180px] z-10 border-2 border-main-color right-10 top-4 bg-white rounded shadow-md">
              ...
              <div className="overflow-y-scroll max-h-[150px]">
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => {
                    ...
                    return (
                      <div
                        className="flex justify-start gap-x-3 border-b-2 border-main-color"
                        key={index}
                      >
                        <div className="w-[180px] h-[25px] bg-main-color mt-[0.35rem] ml-[0.5rem] rounded-md">
                          <h1 className="flex justify-center items-center text-[1rem]">
                            {index + 1}
                          </h1>
                        </div>
                        <div className="w-[750px] grid grid-col justify-start">
                          ...
                          <h1 className="text-black relative left-[14px]">
